...
check.addEventListener("click", () => {
    let key = `0ff1fbf86d9f2246dc7d98480fae4b13`;
    ...
    fetch(url).then(response => {
        ...
        document.body.style.backgroundImage ="url('https://source.unsplash.com/1600x1800/?" + "')";

        data.weather.forEach(items => {
            weatherDescription.innerText = items.description;
            if (items.id < 250) {
                tempIcon.src = `imageicon/storm.svg`;
            } else if (items.id < 350) {
                tempIcon.src = `imageicon/drizzle.svg`;
            } else if (items.id < 550) {
                tempIcon.src = `imageicon/snow.svg`;
            } else if (items.id < 650) {
                tempIcon.src = `imageicon/rain.svg`;
            } else if (items.id < 800) {
                tempIcon.src = `imageicon/atmosphere.svg`;
            } else if (items.id === 800) {
                tempIcon.src = `imageicon/sun.svg`;
            } else if (items.id > 800) {
                tempIcon.src = `imageicon/clouds.svg`;
            }
        })
